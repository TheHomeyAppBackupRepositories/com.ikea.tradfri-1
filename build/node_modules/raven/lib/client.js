...
const nodeUtil = require('util'); // nodeUtil to avoid confusion with "utils"
...
const stringify = require('../vendor/json-stringify-safe');

const instrumentor = require('./instrumentation/instrumentor');
...
extend(Raven.prototype, {
  ...
  install: function install(cb) {
    ...
    if (this.captureUnhandledRejections) {
      ...
      global.process.on('unhandledRejection', (reason, promise) => {
        ...
        self.captureException(reason, context, (sendErr, eventId) => {
          if (!sendErr) {
            const reasonMessage = (reason && reason.message) || reason;
            utils.consoleAlert(
  ...
  makeErrorHandler() {
    ...
    return function(err) {
      if (!caughtFirstError) {
        ...
        self.captureException(err, { level: 'fatal' }, (sendErr, eventId) => {
          if (!calledFatalError) {
  ...
  process: function process(eventId, kwargs, cb) {
    ...
    if (Object.keys(kwargs.request).length === 0) {
      ...
      if (Object.keys(req).length > 0) {
        const parseUser = Object.keys(kwargs.user).length === 0 ? this.parseUser : false;
  ...
  captureException: function captureException(err, kwargs, cb) {
    ...
    const self = this;
    const eventId = this.generateEventId();
    parsers.parseError(err, kwargs, kw => {
  ...
  setCallbackHelper(propertyName, callback) {
  ...
  _createRequestObject() {
     ...
     * need all of them.
     * */
    let sources = Array.from(arguments).filter(source => {
