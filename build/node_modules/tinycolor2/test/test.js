...
test('With Ratio', () => {
  ...
  equal(tinycolor.fromRatio({
    r: 1, g: 0, b: 0, a: 0.5,
  }).toRgbString(), 'rgba(255, 0, 0, 0.5)', 'alpha works when ratio is parsed');
  equal(tinycolor.fromRatio({
    r: 1, g: 0, b: 0, a: 1,
  }).toRgbString(), 'rgb(255, 0, 0)', 'alpha = 1 works when ratio is parsed');
  equal(tinycolor.fromRatio({
    r: 1, g: 0, b: 0, a: 10,
  }).toRgbString(), 'rgb(255, 0, 0)', 'alpha > 1 works when ratio is parsed');
  equal(tinycolor.fromRatio({
...
test('Without Ratio', () => {
  ...
  equal(tinycolor({ r: 0.1, g: 0.1, b: 0.1 }).toHexString(), '#000000', '000000');
  equal(tinycolor('rgb .1 .1 .1').toHexString(), '#000000', '000000');
});

module('String Parsing');

test('RGB Text Parsing', () => {
  ...
  deepEqual(tinycolor({ r: 255, g: 0, b: 0 }).toRgb(), {
    r: 255, g: 0, b: 0, a: 1,
...
test('Percentage RGB Text Parsing', () => {
  ...
  deepEqual(tinycolor({ r: '100%', g: '0%', b: '0%' }).toRgb(), {
    r: 255, g: 0, b: 0, a: 1,
  }, 'object input and compare');
...
test('HSL parsing', () => {
  equal(tinycolor({ h: 251, s: 100, l: 0.38 }).toHexString(), '#2400c2', 'to hex');
  equal(tinycolor({ h: 251, s: 100, l: 0.38 }).toRgbString(), 'rgb(36, 0, 194)', 'to rgb');
  equal(tinycolor({ h: 251, s: 100, l: 0.38 }).toHslString(), 'hsl(251, 100%, 38%)', 'to hsl');
...
test('Invalid alpha should normalize to 1', () => {
  equal(tinycolor({
    r: 255, g: 20, b: 10, a: -1,
  }).toRgbString(), 'rgb(255, 20, 10)', 'Negative value');
  equal(tinycolor({
    r: 255, g: 20, b: 10, a: -0,
  }).toRgbString(), 'rgba(255, 20, 10, 0)', 'Negative 0');
  equal(tinycolor({
    r: 255, g: 20, b: 10, a: 0,
  }).toRgbString(), 'rgba(255, 20, 10, 0)', '0');
  equal(tinycolor({
    r: 255, g: 20, b: 10, a: 0.5,
  }).toRgbString(), 'rgba(255, 20, 10, 0.5)', '.5');
  equal(tinycolor({
    r: 255, g: 20, b: 10, a: 1,
  }).toRgbString(), 'rgb(255, 20, 10)', '1');
  equal(tinycolor({
    r: 255, g: 20, b: 10, a: 100,
  }).toRgbString(), 'rgb(255, 20, 10)', 'Greater than 1');
  equal(tinycolor({
    r: 255, g: 20, b: 10, a: 'asdfasd',
  }).toRgbString(), 'rgb(255, 20, 10)', 'Non Numeric');
...
test('toString() with alpha set', () => {
  const redNamed = tinycolor.fromRatio({
    r: 255, g: 0, b: 0, a: 0.6,
  }, { format: 'name' });
  const transparentNamed = tinycolor.fromRatio({
    r: 255, g: 0, b: 0, a: 0,
  }, { format: 'name' });
  const redHex = tinycolor.fromRatio({
    r: 255, g: 0, b: 0, a: 0.4,
  }, { format: 'hex' });
...
test('setting alpha', () => {
  ...
  equal(hexSetter.getAlpha(), 0.9, 'setAlpha should change alpha value');
  hexSetter.setAlpha(0.5);
  equal(hexSetter.getAlpha(), 0.5, 'setAlpha should change alpha value');
...
test('Alpha = 0 should act differently on toName()', () => {
  equal(tinycolor({
    r: 255, g: 20, b: 10, a: 0,
  }).toName(), 'transparent', '0');
...
test('isDark returns true/false for dark/light colors', () => {
...
test('isLight returns true/false for light/dark colors', () => {
...
test('HSL String', () => {
  for (let i = 0; i < conversions.length; i++) {
    ...
    const output = tinycolor(tiny.toHslString()).toRgb();
    const maxDiff = 2;
...
test('HSV String', () => {
  for (let i = 0; i < conversions.length; i++) {
    ...
    const tiny = tinycolor(c.hex);
    const input = tiny.toRgb();
    const output = tinycolor(tiny.toHsvString()).toRgb();
    const maxDiff = 2;
...
test('PRGB Object', () => {
  for (let i = 0; i < conversions.length; i++) {
    ...
    const tiny = tinycolor(c.hex);
    const input = tiny.toRgb();
    const output = tinycolor(tiny.toPercentageRgb()).toRgb();
    const maxDiff = 2;
...
test('PRGB String', () => {
  for (let i = 0; i < conversions.length; i++) {
    ...
    const tiny = tinycolor(c.hex);
    const input = tiny.toRgb();
    const output = tinycolor(tiny.toPercentageRgbString()).toRgb();
    const maxDiff = 2;
...
test('Mix', () => {
  ...
  // with colors
  for (var i = 0; i < 100; i++) {
    let new_hex = Math.round((255 * (100 - i)) / 100).toString(16);
