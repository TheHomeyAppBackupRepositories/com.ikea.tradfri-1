...
const { table } = coloroid;
const angleTable = [].concat(table.slice(-13), table.slice(0, -13));
...
const Xn = xyz.whitepoint[o][i][0];
const Yn = xyz.whitepoint[o][i][1];
const Zn = xyz.whitepoint[o][i][2];

const y0 = Xn / (Xn + Yn + Zn);
const x0 = Yn / (Xn + Yn + Zn);
const ew = (Xn + Yn + Zn) / 100;
...
xyy.coloroid = function(arg) {
  ...
  // calc saturation
  const yl = row[4]; const el = row[2]; const
    xl = row[3];

  // yl should be scaled to 0..100;
  const Yl = yl * el * 100;

  const T = 100 * Y * (x0 * ew - x * ew) / (100 * (x * el - xl * el) + Yl * (x0 * ew - x * ew));
...
coloroid.xyy = function(arg) {
  const A = arg[0]; const T = arg[1]; const
    V = arg[2];
  ...
  const x = (100 * Y * x0 * ew + 100 * xl * el * T - Yl * T * x0 * ew) / (100 * T * el - Yl * T * ew + 100 * Y * ew);
  const y = (100 * Y + 100 * T * yl * el - Yl * T) / (Y * ew * 100 + T * 100 * el - T * Yl * ew);
