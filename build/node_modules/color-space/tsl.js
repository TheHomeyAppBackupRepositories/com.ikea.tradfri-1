...
tsl.rgb = function(tsl) {
  ...
  let x = Math.tan(2 * Math.PI * (T - 1 / 4));
  x *= x;

  const r = Math.sqrt(5 * S * S / (9 * (1 / x + 1))) + 1 / 3;
  const g = Math.sqrt(5 * S * S / (9 * (x + 1))) + 1 / 3;

  const k = L / (0.185 * r + 0.473 * g + 0.114);
...
rgb.tsl = function(rgb) {
  ...
  const r_ = (r / (r + g + b) || 0) - 1 / 3;
  const g_ = (g / (r + g + b) || 0) - 1 / 3;
  const T = g_ > 0 ? 0.5 * Math.atan(r_ / g_) / Math.PI + 0.25
    : g_ < 0 ? 0.5 * Math.atan(r_ / g_) / Math.PI + 0.75 : 0;
