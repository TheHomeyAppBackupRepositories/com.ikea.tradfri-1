...
class BatteryAlarmUtil {
  ...
  async initialize() {
    ...
    configureBatteryReporting().catch(() => {
      // If it fails try again once on the next incoming ZCL frame
      this.device.node.once('handleFrame', () => configureBatteryReporting.catch(this.device.error));
    });
